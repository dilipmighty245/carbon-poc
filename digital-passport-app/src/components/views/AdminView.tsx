...
export const AdminView: React.FC = () => {
  ...
  const kpis = [
    { title: 'Active Users', val: '128', subtitle: '96% MFA enabled', color: 'text-emerald-600' },
    { title: 'Audit Events', val: '18,442', subtitle: 'Last 30 days', color: 'text-emerald-600' },
    { title: 'API Success', val: '99.92%', subtitle: '7 active clients', color: 'text-emerald-600' },
    { title: 'Backup Status', val: 'Healthy', subtitle: 'Last restore test passed', color: 'text-emerald-600' },
  ];
  ...
  const records = [
    { name: 'Santhosh Samudrala', action: 'Issued passport CP-GH-2026-00481', time: '13:42:18', hash: 'evt_88cd21' },
    { name: 'Verifier A. Mensah', action: 'Approved verification opinion', time: '12:18:04', hash: 'evt_76ab09' },
    { name: 'System Integration', action: 'Imported SAP production extract', time: '11:05:36', hash: 'evt_61bd73' },
    { name: 'Facility Manager', action: 'Corrected meter gap record', time: '09:47:21', hash: 'evt_44ef16' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-xs font-semibold text-slate-500">
          <span className="hover:text-slate-800">Administration</span>
          <span>/</span>
          <span className="text-slate-900 font-bold">Platform Administration</span>
        </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium">Tenant controls, audit, APIs, configuration, system health and recovery</p>
        </div>
      ...
      {/* Middle Row (2/3 Chart + 1/3 Control status) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left 2/3: System activity and API health */}
        <div className="lg:col-span-2 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <h3 className="font-bold text-slate-900 text-sm mb-4">System activity and API health</h3>
      ...
      {/* Bottom Card: Immutable audit events */}
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-slate-900 text-sm">Immutable audit events</h3>
        ...
        <div className="divide-y divide-slate-100 text-xs">
          {records.map((r, i) => (
            <div key={i} className="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="w-28 font-mono text-[11px] text-emerald-700 font-semibold bg-emerald-50 px-2 py-0.5 rounded text-center">
                <span>{r.time}</span>
              </div>
              <div className="text-right font-mono text-slate-400 text-[11px] w-28">
                <span>{r.hash}</span>
              </div>
            </div>
          ))}
