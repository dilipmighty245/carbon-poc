...
export const SuppliersView: React.FC = () => {
  ...
  const kpis = [
    { title: 'Active Suppliers', val: '46', subtitle: '31 primary-data enabled', color: 'text-emerald-600' },
    { title: 'Response Rate', val: '82%', subtitle: '↑ 9% this quarter', color: 'text-emerald-600' },
    { title: 'Verified PCFs', val: '27', subtitle: '59% supplier coverage', color: 'text-emerald-600' },
    { title: 'Incomplete Records', val: '4', subtitle: 'Correction required', color: 'text-amber-600' },
  ];
  ...
  const records = [
    { name: 'Aqua Packaging Ghana', category: 'Packaging', status: '86% COMPLETE', statusClass: 'bg-emerald-50 text-emerald-700 border-emerald-100', action: 'CORRECTION' },
    { name: 'Volta Agro Inputs', category: 'Cocoa inputs', status: '72% COMPLETE', statusClass: 'bg-emerald-50 text-emerald-700 border-emerald-100', action: 'ASSUMPTION' },
    { name: 'West Coast Logistics', category: 'Freight', status: '64% COMPLETE', statusClass: 'bg-emerald-50 text-emerald-700 border-emerald-100', action: 'OVERDUE' },
    { name: 'Tema Process Chemicals', category: 'Chemicals', status: '58% COMPLETE', statusClass: 'bg-emerald-50 text-emerald-700 border-emerald-100', action: 'INCOMPLETE' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Supplier & Customer Network</h1>
          <p className="text-xs text-slate-500 font-medium">Primary carbon data, declarations, evidence and improvement workflow</p>
        </div>
      ...
      {/* Middle Row (2/3 Contribution Chart + 1/3 Incomplete suppliers) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left 2/3: Supplier emissions and completeness */}
        <div className="lg:col-span-2 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <h3 className="font-bold text-slate-900 text-sm mb-4">Supplier emissions and completeness</h3>
        ...
        <div className="divide-y divide-slate-100 text-xs">
          {records.map((r, i) => (
            <div key={i} className="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="text-right font-medium text-slate-400 text-[11px] w-32 uppercase tracking-wider">
                <span>{r.action}</span>
              </div>
            </div>
          ))}
