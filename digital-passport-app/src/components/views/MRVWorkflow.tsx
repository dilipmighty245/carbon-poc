...
export const MRVWorkflow: React.FC = () => {
  ...
  const kpis = [
    { title: 'Readiness Score', val: '91%', subtitle: '2 blockers remaining', color: 'text-emerald-600' },
    { title: 'Evidence Items', val: '186', subtitle: '179 accepted', color: 'text-emerald-600' },
    { title: 'Open Findings', val: '3', subtitle: '1 potentially material', color: 'text-amber-600' },
    { title: 'Verifier Status', val: 'In Review', subtitle: 'Engagement VER-026', color: 'text-slate-900' },
  ];
  ...
  const records = [
    { code: 'F-021', name: 'Grid meter gap - 4 hours', severity: 'MINOR', statusClass: 'bg-emerald-50 text-emerald-700 border-emerald-100', action: 'Correction submitted' },
    { code: 'F-018', name: 'Supplier factor expired', severity: 'MATERIAL', statusClass: 'bg-amber-50 text-amber-700 border-amber-100', action: 'Open' },
    { code: 'F-014', name: 'Allocation evidence incomplete', severity: 'MINOR', statusClass: 'bg-emerald-50 text-emerald-700 border-emerald-100', action: 'Verifier review' },
    { code: 'F-009', name: 'Duplicate freight row', severity: 'RESOLVED', statusClass: 'bg-slate-100 text-slate-700 border-slate-200', action: 'Closed' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-xs font-semibold text-slate-500">
          <span className="hover:text-slate-800">MRV & Verification</span>
          <span>/</span>
      ...
      {/* Middle Row (2/3 Chart + 1/3 Verification gates) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left 2/3: Evidence completeness by area */}
        <div className="lg:col-span-2 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <h3 className="font-bold text-slate-900 text-sm mb-4">Evidence completeness by area</h3>
        ...
        <div className="divide-y divide-slate-100 text-xs">
          {records.map((r, i) => (
            <div key={i} className="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="text-right font-medium text-slate-500 text-[11px] w-48">
                <span>{r.action}</span>
              </div>
            </div>
          ))}
